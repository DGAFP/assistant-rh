...
```bash
moon query changed-files
moon query changed-files --status modified,staged
moon query changed-files --base main --head HEAD
```
...
**Projects:** project, projectAlias, projectSource, projectLayer, language, layer, stack, tag
...
```bash
moon query projects "language=typescript && projectLayer=library"
