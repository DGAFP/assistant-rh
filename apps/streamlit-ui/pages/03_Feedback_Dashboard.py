...
with st.sidebar:
    ...
    if _pc1.button("🧪 Beta-test", width="stretch", help="8 jan – 6 fév 2026"):
        st.session_state["fb_date_range"] = (BETA_START, BETA_END)
        st.rerun()
    if _pc2.button("📅 Tout 2026", width="stretch"):
        st.session_state["fb_date_range"] = (date(2026, 1, 1), date.today())
        st.rerun()
    
    if "date" in df_questions.columns and df_questions["date"].notna().any():
        min_date = df_questions["date"].min()
        max_date = df_questions["date"].max()
        effective_max = max_date if max_date else date.today()
        
        default_range = st.session_state.get("fb_date_range", (date(2026, 1, 1), effective_max))
        
        date_range = st.date_input(
            "Période d'analyse",
            value=default_range,
            min_value=min_date if min_date else BETA_START,
    ...
    exclude_missing_docs = st.checkbox(
        "Exclure 'Document manquant'",
        value=False,
        help="Exclut les feedbacks où le problème est un document manquant dans la base"
    )
    exclude_hors_perimetre = st.checkbox(
        "Exclure 'Hors périmètre'",
        value=False,
        help="Exclut les feedbacks dont le périmètre beta-test est 'Non'"
    )
...
mask_hors = (df_f["beta_scope"].fillna("").astype(str).str.strip().str.lower() == "non") if "beta_scope" in df_f.columns else pd.Series(False, index=df_f.index)
...
with col1:
    st.metric(
        "💬 Questions totales",
        len(df_q),
        help="Nombre de questions posées par les groupes sélectionnés"
    )
...
with col2:
    st.metric(
        "📝 Évaluations",
        len(df_f),
        help="Nombre de feedbacks reçus"
    )
...
with col3:
    if len(df_f) > 0 and "stars" in df_f.columns and df_f["stars"].notna().any():
        ...
        st.metric(
            "⭐ Note moyenne",
            f"{avg_stars:.1f}/5",
            help="Note moyenne sur les évaluations avec étoiles"
        )
    else:
...
with col4:
    if "session_id" in df_q.columns and df_q["session_id"].notna().any():
        ...
        st.metric(
            "👥 Testeurs uniques",
            unique_testers,
            help="Nombre d'utilisateurs uniques (basé sur session_id persistant)"
        )
    else:
...
with col5:
    if len(df_q) > 0 and len(df_f) > 0:
        ...
        st.metric(
            "📊 Taux d'évaluation",
            f"{eval_rate:.1f}%",
            help="Pourcentage de questions ayant reçu un feedback"
        )
    else:
...
with chart_col4:
    ...
    if "user_group" in df_q.columns:
        ...
        fig = px.pie(
            ...
            names=[get_group_label(g) for g in group_counts.index],
            hole=0.4,
...
if "theme" in df_f.columns and df_f["theme"].notna().any():
    ...
    fig = px.pie(
        ...
        names=theme_labels,
        hole=0.4,
        color_discrete_map=theme_pie_colors
    )
...
if df_f.empty:
    ...
    if "ai_reason" in display_df.columns:
        display_df["ai_reason_display"] = display_df["ai_reason"].apply(
    ...
    display_cols = ["ts", "label", "groupe_display", "question", "answer",
                    "reasons_positive", "reasons_negative", "comment",
                    "error_label", "ai_reason_display", "theme_display", "beta_scope_display"]
    ...
    feedback_export_df = feedback_display_df.rename(
        columns={col: export_labels[col] for col in feedback_display_df.columns if col in export_labels}
    )
...
with export_col2:
    ...
    if "ts" in questions_export_df.columns:
        questions_export_df["ts"] = pd.to_datetime(
            questions_export_df["ts"], utc=True, errors="coerce"
        ).dt.tz_convert("Europe/Paris").dt.strftime("%d/%m/%Y %H:%M")
    try:
...
if not df_q_usage.empty and "session_id" in df_q_usage.columns:
    ...
    with usage_col1:
        ...
        user_counts = df_q_usage.groupby("session_id").agg(
    ...
    with usage_col2:
        ...
        if "date" in df_q_usage.columns:
            ...
            fig_ret.add_trace(go.Scatter(name="Questions", x=weekly["semaine"], y=weekly["questions"], yaxis="y2", mode="lines+markers", line=dict(color="#00CC96")))
