...
def load_goldset_from_db():
    ...
    ``run_count`` compte les passages de chaque question dans le système
    d'éval actuel (``rag_quality_eval_items``, alimenté par run-rag-eval) ;
    l'ancienne table ``goldset_runs`` n'existe plus sur staging/prod.
    """
    from sqlalchemy import text as sa_text

    from src.ui.db_utils import missing_tables

    engine = get_engine()
    if not engine:
        return None

    if missing_tables(engine, ["rag_quality_eval_items"]):
        run_count_sql = "0 as run_count"
    else:
        run_count_sql = "(SELECT COUNT(*) FROM rag_quality_eval_items ri WHERE ri.question_id = gq.id) as run_count"
...
def update_question_tags(question_ids: list, tags: list, mode: str = "set"):
    ...
    try:
        with conn.cursor() as cur:
            if mode == "set":
                cur.execute("UPDATE goldset_questions_v2 SET tags = %s WHERE id = ANY(%s)", (tags, question_ids))
            elif mode == "add":
...
def update_question_difficulty(question_ids: list, difficulty: str):
    ...
    try:
        with conn.cursor() as cur:
            cur.execute("UPDATE goldset_questions_v2 SET difficulty = %s WHERE id = ANY(%s)", (difficulty, question_ids))
        return True
    except Exception as e:
...
def update_question_goldset_name(question_ids: list, new_goldset_name: str):
    ...
    try:
        with conn.cursor() as cur:
            cur.execute("UPDATE goldset_questions_v2 SET goldset_name = %s WHERE id = ANY(%s)", (new_goldset_name, question_ids))
        return True
    except Exception as e:
...
if selected_tags:
    tag_filter_mode = st.sidebar.radio("Mode filtre tags", ["Contient au moins un", "Contient tous"], horizontal=True)
...
if selected_tags:
    if tag_filter_mode == "Contient tous":
        df_filtered = df_filtered[df_filtered["tags"].apply(lambda tags: isinstance(tags, list) and all(t in tags for t in selected_tags))]
    else:
        df_filtered = df_filtered[df_filtered["tags"].apply(lambda tags: isinstance(tags, list) and any(t in tags for t in selected_tags))]

if show_untagged:
    df_filtered = df_filtered[df_filtered["tags"].apply(lambda t: not isinstance(t, list) or len(t) == 0) & df_filtered["difficulty"].isna()]
...
tab_browse, tab_categorize, tab_stats = st.tabs(["📋 Questions", "🏷️ Catégorisation", "📊 Stats par capacité"])
...
with tab_browse:
    ...
    df_display["tags_str"] = df_display["tags"].apply(lambda t: ", ".join(t) if isinstance(t, list) and t else "")

    display_cols = ["id", "question", "theme", "tags_str", "goldset_name", "gold_sources", "source"]
    ...
    st.dataframe(df_display[display_cols], width="stretch", hide_index=True, height=600, column_config=column_config)
...
with tab_categorize:
    ...
    df_select["tags_str"] = df_select["tags"].apply(lambda t: ", ".join(t) if isinstance(t, list) and t else "")
    df_select["question_short"] = df_select["question"].str[:120]

    # Selection mode
    selection_mode = st.radio("Mode de sélection", ["Sélection manuelle", "Toutes les questions filtrées"], horizontal=True)
    ...
    if n_selected == 0:
        ...
        with col_action1:
            ...
            new_difficulty = st.selectbox(
                "Assigner une difficulté", ["(ne pas changer)", "easy", "medium", "hard", "(effacer)"], key="bulk_difficulty"
            )
        ...
        with col_action2:
            st.markdown("#### 🏷️ Tags")
            tag_action = st.radio("Action sur les tags", ["Ajouter", "Retirer", "Remplacer"], horizontal=True, key="tag_action_mode")

            # Merge predefined + existing + allow custom
            all_available_tags = get_all_tags_in_db(df_goldset)
            chosen_tags = st.multiselect("Tags à appliquer", all_available_tags, key="bulk_tags")

            # Custom tag input
            custom_tag = st.text_input("Ou créer un tag custom", key="custom_tag", placeholder="ex: mon-tag-custom")
            if custom_tag:
            ...
            if chosen_tags:
                ...
                if st.button(f"{tag_action} {len(chosen_tags)} tag(s) sur {n_selected} question(s)", key="btn_tags"):
        ...
        with col_gs1:
            new_gs_name = st.text_input("Nouveau goldset_name", key="new_goldset_name", placeholder="ex: eval_acronyms_v1")
        with col_gs2:
            if new_gs_name and st.button(f"Assigner à {n_selected} question(s)", key="btn_goldset_name"):
        ...
        q_options = {
            f"Q{row['id']}: {row['question'][:100]}": row["id"] for _, row in df_filtered[df_filtered["id"].isin(selected_ids)].head(50).iterrows()
        }
        if q_options:
            ...
            with col_e2:
                ...
                edit_tags = st.multiselect(
                    f"Tags (actuels: {', '.join(current_tags) or 'aucun'})", all_available_tags, default=current_tags, key=f"edit_tags_{chosen_q_id}"
                )
...
with tab_stats:
    ...
    with col_s1:
        ...
        if tag_counter:
            df_tags = pd.DataFrame(sorted(tag_counter.items(), key=lambda x: -x[1]), columns=["Tag", "Questions"])
    ...
    df_untagged = df_stats[df_stats["tags"].apply(lambda t: not isinstance(t, list) or len(t) == 0) & df_stats["difficulty"].isna()]
