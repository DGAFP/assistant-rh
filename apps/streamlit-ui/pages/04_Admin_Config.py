...
def _run_health_check_inline() -> dict:
    ...
    try:
        ...
        if key:
            r = requests.get("https://albert.api.etalab.gouv.fr/v1/models", headers={"Authorization": f"Bearer {key}"}, timeout=5)
            health["Albert"] = {
...
with tab_config:
    ...
    with col_v3_right:
        ...
        new_selector = st.toggle(
            "Activer LLM Selector", value=current_selector, help="Filtre et priorise les sections pertinentes via un LLM", key="toggle_v3_selector"
        )
    ...
    with col_gen2:
        new_v3_temperature = st.slider(
            ...
            min_value=0.0,
            max_value=1.5,
            value=getattr(config, "v3_temperature", 0.0),
            step=0.1,
    ...
    with col_p2:
        ...
        if selector_prompts:
            ...
            new_selector_prompt = st.selectbox(
                "Prompt Selector", selector_prompts, index=selector_idx, help="Sélection des passages pertinents", key="select_v3_selector_prompt"
            )
    ...
    with col_debug1:
        new_verbose = st.toggle("📋 Mode Verbose", value=config.verbose_mode, help="Affiche les logs détaillés", key="toggle_verbose_v3")
    ...
    with col_editor:
        ...
        if selected == "__new__":
            ...
            if content:
                ...
                # Content editor
                edited_content = st.text_area("Contenu", value=content, height=400, key=f"textarea_content_{selected}")
                ...
                with col_delete:
                    if selected != "system_prompt.md":  # Can't delete default
                        ...
                        st.button(
                            ...
                            width="stretch",
                            disabled=True,
                            help="Impossible de supprimer le prompt par défaut",
                            key="btn_del_default_disabled",
                        )
...
with tab_acronyms:
    ...
    with col_list:
        ...
        if search_query:
            ...
            filtered_acronyms = [a for a in acronyms if search_lower in a.acronym.lower() or search_lower in a.expansion.lower()]
        else:
            ...
            if not df.equals(edited_df):
                for idx, row in edited_df.iterrows():
                    ...
                    if row["Expansion"] != original["Expansion"] or row["Catégorie"] != original["Catégorie"]:
                        update_acronym(acronym=row["Acronyme"], expansion=row["Expansion"], category=row["Catégorie"])
    ...
    with col_form:
        ...
        acronym_to_delete = st.selectbox("Sélectionner l'acronyme à supprimer", options=[a.acronym for a in acronyms], key="acronym_delete_select")
    ...
    st.caption(
        "Acronymes détectés dans les requêtes utilisateur mais absents du dictionnaire. Utilisez le formulaire ci-dessus pour les ajouter, ou marquez-les comme traités."
    )
                ...
                [
                    {
                        ...
                        "Exemple de requête": (m.get("sample_query", "")[:] + "...")
                        if m.get("sample_query") and len(m.get("sample_query", "")) > 60
                        else m.get("sample_query", "-"),
                    }
