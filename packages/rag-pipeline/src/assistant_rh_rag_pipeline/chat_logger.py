...
JSONB_COLUMNS: set[str] = {
    "filters", "retrieved", "sources_used_content",
    "dist_before_rerank", "dist_after_rerank", "boost_weights",
    "chunks_sent_to_selector",
    "v3_legal_refs_details", "v3_timing_breakdown",
    "v3_context_items_summary", "v3_source_distribution",
    "v3_context_items_full", "v3_selector_input_items", "v3_selector_decisions",
    "v3_chunks_raw", "v3_sections_raw", "v3_context_before_selector",
    "v3_retrieval_params", "v3_chunks_before_rerank", "v3_chunks_after_rerank",
    "v3_aggregation_params", "v3_sections_before_rerank", "v3_sections_after_rerank",
}
...
def _build_upsert_sql(data: dict) -> str:
    ...
    return (
        f"INSERT INTO chat_runs ({col_names}) VALUES ({', '.join(values)}) "
        f"ON CONFLICT (turn_id) DO UPDATE SET {update_set}"
    )
...
def build_log_row(
    ...
    row.update({
        ...
        "v3_selector_kept_indices": ",".join(
            str(d["idx"]) for d in selector_decisions.get("kept", [])
        ),
        "v3_selector_removed_indices": ",".join(
            str(d["idx"]) for d in selector_decisions.get("removed", [])
        ),
    ...
    row.update({
        ...
        "v3_legal_refs_details": _jdumps([
            {"number": num, "cid": info.get("cid", ""), "title": info.get("title", "")}
            for num, info in resolved_refs.items()
        ]) or "[]",
...
def build_non_rag_row(
    ...
    return {
        ...
        "system_prompt": "",
        "system_prompt_name": "",
        "use_query_rewriting": False, "rewritten_query": "",
        "use_hyde": False, "hyde_document": "",
        "use_query_expansion": False, "expanded_query": "",
        "acronyms_used": ",".join(qr.expanded_acronyms or []),
        "query_for_retrieval": "",
        "retrieval_mode": "", "hybrid_alpha": 0, "sparse_method": "",
        "session_id": session_state.get("session_id", ""),
        "conversation_id": session_state.get("conversation_id", ""),
        "turn_index": len(session_state.get("turns", [])),
        "sources_used_count": 0, "sources_used_indices": "",
        "sources_used_content": "[]",
        "fallbacks_used": "", "sources_raw_line": "",
        "retrieval_time_ms": 0, "rerank_time_ms": 0,
        "llm_time_ms": 0, "total_time_ms": 0,
        "ttft_ms": 0, "tokens_per_second": 0.0,
        ...
        "intent_model": "",
        "use_query_reformulation": False, "reformulated_query": "",
        "reformulation_model": "",
        ...
        "user_group": session_state.get("user_group", "default"),
        "llm_selector_model": "", "llm_selector_prompt_name": "",
        "llm_selector_reasoning": "", "llm_selector_time_ms": 0,
        "chunks_sent_to_selector": "[]", "llm_selector_response": "",
        "pick_mode": "", "chunks_before_pick": 0, "chunks_after_pick": 0,
...
def _prepare_data(data: dict) -> dict:
    ...
    for col in ("filters", "retrieved", "sources_used_content",
                "boost_weights", "dist_before_rerank", "dist_after_rerank"):
        val = out.get(col)
    ...
    for col in ("llm_selector_model", "llm_selector_prompt_name", "llm_selector_reasoning",
                "pick_mode", "intent_gating_prompt_name",
                "chunks_sent_to_selector", "llm_selector_response"):
        if col not in out:
            out[col] = None
