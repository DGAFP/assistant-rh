"""Gold partagé: sections silver -> chunks SECTION_ATOMIC + embeddings.

Contrairement à Service-Public (structure QNA du XML), les documents OCR n'ont
que le sectionnement heading-based: chaque section indexable est découpée en
chunks SECTION_ATOMIC. Le champ source vaut identity.chunk_source — jamais une
constante partagée (le hardcode qui a fui dans MATTE). Le chunking table-aware
(couture des tableaux coupés par page, re-port d'en-tête) vient de l'audit du
corpus MI réel (2026-07-04). Un moteur gold alternatif (QNA matte/mso) peut
remplacer ce builder tant qu'il produit le même contrat de lignes chunks.
"""
...
class SectionAtomicGoldBuilder:
    def __init__(self, identity: MinistryIdentity, embedding_config: EmbeddingConfig, gold_config: GoldConfig):
    ...
    def build_chunks(
        ...
        for section in sections:
            ...
            qa_id = hashlib.sha1(f"section:{section_id}".encode("utf-8")).hexdigest()  # lgtm[py/weak-sensitive-data-hashing]
            texts = split_section_markdown(
                section_markdown,
                self.gold_config.chunk_max_chars,
                self.gold_config.chunk_overlap,
            ) or [section_markdown]
            if self.gold_config.min_chunk_payload_chars > 0:
                texts = [text for text in texts if len(_chunk_payload(text)) >= self.gold_config.min_chunk_payload_chars]
