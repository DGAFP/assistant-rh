"""Silver partagé: markdown OCR -> rag_documents/rag_sections.

Sectionnement heading-based seedé sur le splitter Service-Public. Le parsing
par-ministère peut diverger: un corpus peut fournir son propre builder silver
(le moteur QNA matte/mso de la Phase D) tant qu'il produit le même contrat de
sortie (document + sections).
"""
...
__all__ = ["HeadingSilverBuilder", "SilverBundle", "SilverRepository", "normalize_ocr_markdown"]
...
def section_uuid(identity: MinistryIdentity, doc_id: str, section_index: int) -> str:
    return stable_uuid_from_parts(identity.namespace, doc_id, "section", section_index)
...
class HeadingSilverBuilder:
    """Silver heading-based: markdown OCR -> rag_documents/rag_sections."""

    def __init__(self, identity: MinistryIdentity, config: SilverConfig):
    ...
    def build_bundle(self, asset: BronzeAsset) -> tuple[dict[str, Any], list[dict[str, Any]]]:
        ...
        doc_id = doc_uuid(self.identity, short_id)
        ...
        doc_record = {
            ...
            # VLM — une hallucination d'annotation doit rester distinguable
            # du texte d'origine.
        ...
        for section in sections:
            section_id = section_uuid(self.identity, doc_id, section.section_index)
