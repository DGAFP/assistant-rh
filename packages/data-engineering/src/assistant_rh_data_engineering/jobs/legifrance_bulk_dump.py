...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Échec non-zéro si --article-ids-json demande N LEGIARTI et que "
            "l'extraction (snapshot + deltas) en trouve strictement moins. "
            "Activé par défaut avec --article-ids-json ; utiliser "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Désactive --strict-articles : tolère un --article-ids-json "
            "extrait partiellement (utile pour le mode migration où des "
            "articles LEGIARTI sont déjà connus absents de l'archive)."
        ),
    )
    return parser


def load_article_ids_from_json(path_str: str) -> list[str]:
    path = Path(path_str)
...
def main() -> int:
    ...
    # Échec strict par défaut sur --article-ids-json : évite qu'une extraction
    # incomplète (snapshot partiel + deltas manquants) ne se propage en aval.
    ...
    if args.article_ids_json and missing_ids and strict_articles:
        message = (
            f"Extraction incomplète pour --article-ids-json: "
            f"{len(missing_ids)}/{requested_count} article(s) absent(s) "
            f"du snapshot et des deltas. Premiers manquants: "
            f"{missing_ids[:5]}. Réessayer avec un snapshot plus récent ou "
            f"utiliser --allow-partial pour ignorer."
        )
        print(
            json.dumps(
                {
