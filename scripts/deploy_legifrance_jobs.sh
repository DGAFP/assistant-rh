...
PIPELINE_REMOTE_TAG="${REGISTRY_HOST}/${REGISTRY_NAMESPACE}/legifrance-pipeline:${IMAGE_TAG}"
INGESTION_REMOTE_TAG="${REGISTRY_HOST}/${REGISTRY_NAMESPACE}/legifrance-ingestion:${IMAGE_TAG}"
...
if (( PUSH )); then
  ...
  docker tag "legifrance-pipeline:${IMAGE_TAG}" "${PIPELINE_REMOTE_TAG}"
  docker tag "legifrance-ingestion:${IMAGE_TAG}" "${INGESTION_REMOTE_TAG}"
  docker tag "legifrance-bulk-dump:${IMAGE_TAG}" "${BUILD_DUMP_REMOTE_TAG}"
  docker push "${PIPELINE_REMOTE_TAG}"
  docker push "${INGESTION_REMOTE_TAG}"
  docker push "${BUILD_DUMP_REMOTE_TAG}"
fi

if (( CREATE )); then
  : "${SCW_DEFAULT_PROJECT_ID:?SCW_DEFAULT_PROJECT_ID is required}"
  : "${SCW_ACCESS_KEY:?SCW_ACCESS_KEY is required}"
  TARGET_ENV="${TARGET_ENV}" IMAGE_TAG="${IMAGE_TAG}" ./scripts/create_scaleway_legifrance_dump_job.sh
  TARGET_ENV="${TARGET_ENV}" IMAGE_TAG="${IMAGE_TAG}" ./scripts/create_scaleway_legifrance_job.sh
  TARGET_ENV="${TARGET_ENV}" IMAGE_TAG="${IMAGE_TAG}" ./scripts/create_scaleway_legifrance_ingestion_job.sh
fi
