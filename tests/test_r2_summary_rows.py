...
def test_build_index_variant_tracks_version_and_checksum() -> None:
    v1 = sr.build_index_variant("r2s1-model-p1", "texte A")
    assert v1.startswith("r2_summary/r2s1-model-p1/")
    ...
    assert sr.build_index_variant("r2s2-model-p1", "texte A") != v1
    assert sr.build_index_variant("r2s1-model-p1", "texte B") != v1


def test_build_summary_chunk_row_embeds_summary_serves_authentic_text() -> None:
    row = sr.build_summary_chunk_row(ARTICLE_ROW, SUMMARY, EMBEDDING, summarizer_version="r2s1-model-p1")
    ...
    assert row["index_variant"] == sr.build_index_variant("r2s1-model-p1", ARTICLE_ROW["chunk_text"])
...
def test_build_summary_chunk_row_requires_embedding_and_summary() -> None:
    ...
    with pytest.raises(ValueError, match="embedding"):
        sr.build_summary_chunk_row(ARTICLE_ROW, SUMMARY, [], summarizer_version="v")
    with pytest.raises(ValueError, match="résumé vide"):
        sr.build_summary_chunk_row(ARTICLE_ROW, "  ", EMBEDDING, summarizer_version="v")
    with pytest.raises(ValueError, match="cid"):
        sr.build_summary_chunk_row({**ARTICLE_ROW, "cid": ""}, SUMMARY, EMBEDDING, summarizer_version="v")
...
def test_plan_missing_summaries_delta() -> None:
    ...
    existing = {
        ...
        up_to_date["cid"]: sr.build_index_variant(version, up_to_date["chunk_text"]),
        # Périmé: variant calculé sur l'ANCIEN texte.
        changed["cid"]: sr.build_index_variant(version, "ancien texte"),
    }
    todo = sr.plan_missing_summaries([up_to_date, changed, missing, empty], existing, version)
    assert [row["cid"] for row in todo] == [changed["cid"], missing["cid"]]


def test_plan_missing_summaries_is_stale_on_version_bump() -> None:
    existing = {ARTICLE_ROW["cid"]: sr.build_index_variant("r2s1-model-p1", ARTICLE_ROW["chunk_text"])}
    todo = sr.plan_missing_summaries([ARTICLE_ROW], existing, "r2s2-model-p1")
...
def test_plan_missing_summaries_ignores_summary_rows_in_input() -> None:
    summary_row = {
        **ARTICLE_ROW,
        ...
        "index_variant": sr.build_index_variant("r2s1-model-p1", ARTICLE_ROW["chunk_text"]),
    }
    todo = sr.plan_missing_summaries([summary_row], {}, "r2s1-model-p1")
