...
Sur les 126 colonnes écrites, **72 ne sont relues que par les pages Streamlit admin/debug** (affichage), jamais par du code applicatif ni de la conformance. Beaucoup correspondent à des **features de pipeline retirées (v2)** que le logger remplit encore : `hyde_document`, `use_hyde`, `cascade_source`, `rewritten_query`, `sparse_method`, `pick_mode`, `chunks_before_pick`/`chunks_after_pick`, `boost_weights`… On paie l'écriture et le stockage de concepts morts à chaque tour.
...
| Table interrogée | Lignes (emb m3) | Index vectoriel | Effet |
|---|---|---|---|
| `rag_chunks_service_public` | 1 554 | ✅ ivfflat m3 | ok |
| `rag_chunks_matte` | 959 | ❌ | **scan séquentiel** |
...
| 2 | **Créer les index vectoriels** (ivfflat/hnsw, m3) sur matte, dgafp, rgrh | Latence + coût CPU, prérequis multi-ministère | S |
