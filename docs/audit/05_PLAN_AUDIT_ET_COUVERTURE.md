...
- **Prod jamais auditée directement** : tous les constats viennent d'une copie locale de staging (accès prod non autorisé pendant l'audit). À confirmer sur prod : présence/absence de `rag_chunks_test`, état réel du reranker post-#88, distributions de latence/coût réelles. **Mise à jour (2026-06-15)** : la [note 07](07_VERIFICATION_STAGING_ET_PRIORISATION.md) a re-vérifié l'essentiel sur la **base staging réelle** (DSN actif, lecture seule) — reranker déployé et fonctionnel, `rag_chunks_test` absente, DGAFP 0 embedding, couverture SP refermée (55/55), latences mesurées (~8 s p50 bout-en-bout). **Reste à confirmer sur prod.**
