...
**Référentiel de sources & drift continu (🆕, gouvernance des données).** Le référentiel des sources documentaires (liste + métadonnées : périmètre, autorité, date) vit dans **Grist** (La Suite numérique) et **évolue dans le temps**. Idée : un **diff/drift permanent** entre ce référentiel et le corpus réellement ingéré — repérer (a) une source au référentiel mais absente ou périmée dans le corpus (sous-ingestion), (b) un document ingéré sans entrée au référentiel (orphelin), (c) une source mise à jour depuis la dernière ingestion (fraîcheur). C'est l'**amont** du check de réconciliation (P0.6 / [#114](https://github.com/DGAFP/assistant-rh/pull/114)) : on remonte la chaîne à « entrée référentiel → doc ingéré → chunk → embedding → table interrogée ». Mise en œuvre naturelle : brancher une source Grist (API) sur l'exporter data-health ([#115](https://github.com/DGAFP/assistant-rh/pull/115)) → métriques de drift + alertes, et/ou un gate qualité (#114) bloquant au-delà d'un seuil. Devient un invariant **par périmètre** au multi-ministère (chaque ministère = son référentiel). Cf. note [05](05_PLAN_AUDIT_ET_COUVERTURE.md) D10.
