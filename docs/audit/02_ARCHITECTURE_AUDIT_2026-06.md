...
2. **Schéma de base mal gouverné, partiellement versionné** : le DDL est éclaté — `config/sql/scaleway_postgres_core_schema.sql` versionne `service_public` + `legifrance` (avec leur index ivfflat), mais `matte`/`dgafp`/`rgrh`, `chat_runs` et les fantômes `_scalingo`/`_scw` n'ont aucun DDL en repo, et `supabase/migrations/` ne porte que 2 migrations ; la dérive local/staging/prod est déjà effective (`rag_chunks_test` absente).
3. **Le package critique n'a aucun test colocalisé** : 0 test dans `packages/rag-pipeline/` ; les tests racine couvrent maintenant le contrat `/rerank`, le logging de statut et une partie du déterminisme retriever, mais la couverture reste dispersée et incomplète.
...
**A1 — Python vs Mastra : évaluation délibérée, à trancher ASAP.** Le port Mastra (`apps/mastra-pipeline/`, ~9 850 lignes TS) est un **test assumé** : une **alternative au pipeline Python** (~30 150 lignes) pour s'appuyer sur un **framework optimisé pour l'usage** (agents/RAG). Le choix de cible (Python OU TS) **n'est pas encore tranché — et doit l'être au plus vite** : tant qu'il ne l'est pas, chaque évolution se paie deux fois et la conformance dédiée (6 contrats JSON, baselines, nightly) reste un impôt permanent sur chaque changement de comportement, sensible à l'ordre, ce qui décourage précisément les améliorations de ranking dont la qualité a besoin (cf. note 01). Le port reste par ailleurs incomplet au niveau gateway : le section aggregator appelle bien `/rerank` directement, mais `gateways/albert.ts` indique encore « TODO: wire embedding/reranking resolvers ».
...
**A3 — `rag_config` est un fourre-tout non typé.** ~70 clés JSON dont une majorité de clés v2 mortes (`enable_mmr`, `boost_*`, `dedup_threshold`, `enable_hyde`, `chunk_selection_mode`… 1 seule référence générique dans le code v3). Des valeurs contradictoires y dorment (`v3_token_budget: 8000` alors que le mode WIDE actif utilise 12 000 via getter). Les clés réellement actives ne sont identifiables qu'en lisant le code de la page Chatbot.

**A4 — Schéma de base partiellement versionné, sans source de vérité unique.** Le DDL est éclaté : `config/sql/scaleway_postgres_core_schema.sql` définit `rag_chunks_service_public` + `rag_chunks_legifrance` (avec index ivfflat), tandis que `supabase/migrations/` ne contient que **2 migrations** (bootstrap conformance, mai 2026 ; statut reranker `chat_runs` via [#88](https://github.com/DGAFP/assistant-rh/pull/88), juin 2026). Le reste de la base (~22 tables) n'a aucun DDL en repo : `rag_chunks_matte`/`_dgafp`/`_rgrh`, `chat_runs`, et des fantômes (`rag_chunks_dgafp_scalingo`, `rag_chunks_legifrance_scw`, `rag_chunks_mso` jamais interrogée, `goldset_runs` disparue). Conséquence directe : staging n'a pas `rag_chunks_test` alors que la config l'active (note 01, addendum 1). Il n'existe **aucune source de vérité unique** du schéma attendu.
...
|---|---|---|
| **0 test dans `packages/rag-pipeline/`** | Les tests racine couvrent maintenant le contrat `/rerank`, le logging de statut et une partie du retriever déterministe, mais restent hors package et ne couvrent pas encore embeddings, LLM, aggregator complet ni contrats provider bout-en-bout | La rupture d'API Albert `/rerank` a été corrigée par #88, mais le risque de régression reste trop dépendant de tests dispersés |
...
- **S2 — Injection SQL post-auth dans les pages d'éval.** `09_Pipeline_Evaluation.py:517-545` : `goldset_names`/`tags` interpolés en f-string dans le SQL (`",".join(f"'{g}'")`), à deux endroits quasi identiques. Les valeurs viennent aujourd'hui de la DB elle-même et la page est derrière `require_admin`, mais le motif est un piège. À paramétrer systématiquement. **Correction (2026-06-15)** : contrairement à ce que disait cette note, le motif n'est **pas** recopié dans `11_Golden_Beta_Analysis.py` — cette page utilise des placeholders paramétrés `%s`.
