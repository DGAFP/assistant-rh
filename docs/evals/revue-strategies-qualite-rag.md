...
**Référentiel de mesure** : goldset `baseline_v1` (99 questions réelles résolues), protocole cible à deux étages : **screening** par `x-ai/grok-4.5` via OpenRouter avec ZDR stricte (`provider.data_collection=deny` **et** `provider.zdr=true`) ; **gates d'adoption** par Scaleway `qwen3-235b` souverain en vote majoritaire à 3. Le support opérationnel maj-3 est encore en attente de correction et de merge de la PR #333 ; jusque-là, le workflow courant ne doit pas être utilisé comme gate officiel. Référence officielle construite offline : runs 118/123/124 re-jugés sous Scaleway maj-3 (**0,677**, 26 échecs stables) ; détail dans le journal.
...
| Stratégie | Statut | Preuve / décision |
|---|---|---|
| **Sélection du juge par la mesure (21-22/07)** | 🟡 décision actée, automatisation en attente | Qualification de 5 juges par bruit propre (réponses figées jugées 2x). Verdict : **protocole à 2 étages** — OFFICIEL CIBLE (gates d'adoption) = **Scaleway qwen3-235b souverain en vote majoritaire à 3** (5,1 % single-shot → ~0,8 % maj-3, accord 86,9 % avec l'historique) ; SCREENING = grok-4.5 single-shot (ZDR, 6,1 %). Écartés : qwen3.7-max (aucun endpoint ZDR), gemini-2.5-pro (7,1 %), gpt-5.2 (9,1 % + sur-strict 0,35). **Référence officielle offline = 0,677**, 26 échecs stables = cibles officielles. L'automatisation `--judge-votes 3` reste bloquée sur la PR #333 : ne pas considérer les defaults actuels du workflow comme un gate maj-3. |
...
| 21-22/07 | **Qualification des juges + référence officielle** | 5 juges comparés sur réponses figées 2x ; runs 118/123/124 re-jugés offline sous Scaleway maj-3 | Référence 0,677 ; 26 échecs stables ; automatisation du gate en attente de #333 |
...
1. **Vague 0 — stabilisation de la mesure** : ✅ soldée — référence officielle Scaleway maj-3 construite offline à 0,677, 26 échecs stables et bruit documenté ; automatisation du gate encore bloquée par #333.
...
5. **Zéro-rétention stricte obligatoire** pour tout LLM externe : sur OpenRouter, `data_collection=deny` **et** `zdr=true` sont requis. Les gates d'adoption devront utiliser le juge souverain Scaleway maj-3 dès que #333 sera corrigée et mergée ; jusque-là, aucun run lancé avec les defaults actuels ne doit être présenté comme gate officiel. Albert reste souverain par défaut en prod.
