...
| Routage par catégorie/thème (filtrer ou booster le retrieval sur la catégorie détectée) | 🔴 (sonde-plafond 21/07) | **Le plafond oracle est réel mais aucun chemin d'implémentation n'est net-positif.** Avec la catégorie PARFAITE (taxonomie native : Partie>Livre>Titre CGFP, doc/thématique SP — 252 catégories fines) : filtre dur = **7/7 misses profonds convertis top-30** (rangs 1-28), car **96,7-100 % des concurrents devant le gold sont HORS de sa catégorie** (bruit inter-corpus, surtout SP). MAIS : (a) **boost soft = 0/7** à +0,02/+0,05 de cosinus (le gold est 0,05-0,15 sous la frontière — seule l'élimination convertit, or l'élimination = filtre dur) ; (b) filtre dur sous mauvaise catégorie = **10/10 passers cassés** (miss garanti) → break-even ≥ **90,3 % de précision** sur 252 catégories, depuis des questions métier qui ne nomment jamais le texte — c'est exactement le fossé de vocabulaire qui EST le problème (un classifieur qui le franchirait à 90 % *serait* la solution du retrieval, pas son préfiltre). Pas de zone intermédiaire exploitable. Cas résiduel exploitable sans classifieur : scope explicite choisi par l'UTILISATEUR (feature produit). Piste mécanique ouverte par la sonde : le bruit étant inter-corpus, mesurer le rang du gold DANS SA PROPRE TABLE (données de la sonde) — si ≤13, une fusion à quota par source (mécanique, sans classifieur) mérite une micro-sonde. |
...
| **Graphe de renvois juridiques** (expansion déterministe 1 saut) | 🟢 | Le droit se cite explicitement (« dans les conditions prévues par le décret 84-972 »). Sonde : expansion **bidirectionnelle plafonnée** (≤20/renvoi, ≤15 citeurs) = **6/34 conversions, bruit 22 chunks/q** (9/34 sans plafond ; 26-39 % du sous-ensemble juridique). **La moitié des conversions viennent du sens arrière** (« qui cite le top-30 »). Source : liens PISTE `lien_citations` déjà ingérés (16 636) mais figés à la version d'origine → union avec regex (précision 20/20 vérifiée). Design : table `rag_renvois_edges` à l'ingestion + JOIN post-retrieve, bruit trié par le rerank existant. Zéro LLM. Reste : replay expansion+rerank offline sur les 9 candidats avant d'écrire le code. |
...
| **Protocole de variance** | 🟢 à adopter | Découverte majeure du run 118 : **σ single-shot ≈ ±0,05-0,06** (12 flips aléatoires entre deux runs à config identique, hit_rate=1.0 des deux côtés). Un run unique ne peut PAS mesurer +0,02-0,05 — éclaire rétroactivement la « régression min_kept » du 06/07 et le « SP −0,14 » du run 115 (bruit). Protocole : référence = verdict **majoritaire à 3 runs** par question ; A/B en **diff apparié par question** + re-run des flips ; `--dedupe-scope config-and-git` pour les changements code-only ; goldset gelé pendant tout run. |
...
| 21/07 | **Sonde-plafond routage par catégorie** | Classifieur ORACLE (catégorie du gold) sur taxonomie native (252 catégories fines), rangs cosine exacts plein-scope vs filtré, analyse des concurrents, break-even | 🔴 NO-GO (oracle dur 7/7 mais soft 0/7 et break-even ≥90,3 % irréaliste ; concurrents 96,7-100 % hors-catégorie = bruit inter-corpus) |
