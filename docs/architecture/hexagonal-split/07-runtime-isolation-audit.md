...
| Acronymes figés au constructeur historique ; erreurs SQL → dict vide. | Snapshot frais par appel dans l'API, sans état métier partagé ; `DatabaseFailure`/`DatabaseUnavailable` → détection vide avec diagnostic. Changement de cycle de vie explicite A5-03 ; le cache historique servi reste intact. |
...
| Exception classification → `rag_query`, confiance 0,5, question inchangée, flag juridique false et signal LLM null, aucune expansion de secours. | Même branche, annulation propagée ; diagnostic provider B3 conservé. Les messages réseau historiques sont remplacés par les codes sûrs B3, pas réintroduits dans le core. |
...
| Persistance d'un run | `chat_runs` et `rag_trace_events` sont upsertés séparément, sans relation canonique des sources finales ; l'OTLP part dans un thread daemon. Une réponse peut donc exister sans sources autorisables ou sans événements, ou un processus finir avant export. | `ChatRunStorePort.finalize` atomique pour `chat_runs`, `chat_run_sources` et les traces de toutes les étapes. Export OTLP après commit, best effort mais géré ; en streaming, persister avant le chunk final et `[DONE]`. |
