...
- **Exception admin directe** : la page Streamlit reste protégée par l'auth admin et accède directement à PostgreSQL. À M4, la frontière est bornée par l'allowlist de code ; les credentials DB à privilèges minimaux sont reportés à `admin-hardening`.
...
| Visiteur Streamlit | aucune | catalogue des groupes visibles et création d'une session par mot de passe | Réponses génériques et quotas temporaires sans lockout persistant ; mot de passe jamais logué. |
...
| Administrateur Streamlit | groupe issu du cookie chiffré et revérifié `is_admin` en DB, ou fallback `ADMIN_PASSWORD` d'environnement | pages admin/ops et accès DB explicitement allowlisté dans le code | Questions, réponses, sessions, traces, prompts et gold answers sont sensibles ; les privilèges DB actuels restent une dette explicite jusqu'à `admin-hardening`. |
...
- les tests couvrent ses deux chemins actuels : groupe du cookie chiffré avec `is_admin` revérifié en DB, et fallback `ADMIN_PASSWORD` d'environnement ;
