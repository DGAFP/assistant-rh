...
| D17 | **Les documents internes utilisent une URL signée de quinze minutes, créée au clic** | Seuls `doc_ref`, `turn_id` et l'ordre de la source finale sont persistés. La présignature S3 ne dépasse jamais la durée restante de la capability. Une capability expirée ne se renouvelle pas elle-même : `_PDF_Viewer` la rédime côté serveur, redemande une URL authentifiée et réessaie une fois. |
