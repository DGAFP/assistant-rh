...
| 2026-09-04 | A3 — clôture de relecture de #469 | Perte d'état Streamlit → réauthentification et logout → révocation API actés. Les quotas login sont placés aux frontières Streamlit/API avec backoff temporaire sans lockout de groupe. `_PDF_Viewer` rédime côté serveur pour rendre le retry effectif. L'admin M4 documente ses deux chemins d'auth actuels ; les credentials DB minimaux restent dans `admin-hardening`. |
