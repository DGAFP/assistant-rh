...
| 2026-09-04 | A3 — [issue #444](https://github.com/DGAFP/assistant-rh/issues/444) | [Matrice Streamlit et périmètre API](08-streamlit-api-parity.md) amendés : M4 borne le chemin public HTTP sans DB/pipeline direct ; l'admin/ops Streamlit garde un accès DB allowlisté sans bloquer la bascule ; endpoints admin et RAG-ops reportés ; sessions groupe 8 h, URLs documentaires 15 min créées au clic et feedback étoiles/raisons/commentaire figés. Grafana/Tempo, LangSmith, agentic RAG et retrait du DDL runtime sont consignés comme suites hors chemin critique. |
...
| A5-09 | Des consommateurs utilisent des privés : `09_Pipeline_Evaluation.py` appelle `_retriever/_aggregator/_context_builder`, le logger lit `_context_builder`, le goldset importe `_fold`, le chat importe `_append_csv_row`. | C6 + D3 + F3 | ouvert — ports/runners publics puis retrait du package historique |
...
- **2026-09-04 — admin-hardening** : déplacer le DDL runtime historique vers des migrations, borner les privilèges DB et réévaluer ensuite une extraction vers endpoints admin ou RAG-ops sans bloquer la migration publique.
