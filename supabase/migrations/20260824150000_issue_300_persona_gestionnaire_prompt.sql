-- Issue #300 : persona « gestionnaire RH en SGCD » pour le générateur —
-- ligne ADDITIVE dans system_prompts, candidate d'A/B via
-- `--system-prompt-name` (eval) ou sélection Admin Config.
-- INVARIANT : cette migration ne touche PAS rag_config — le prompt actif
-- reste inchangé tant que l'A/B n'a pas conclu (adoption = migration séparée).
...
VALUES (
    ...
    $prompt_persona$
# Assistant RH – Gestionnaires RH en SGCD (persona gestionnaire)
...
- [ ] La réponse ne répète pas les mêmes informations et ne liste pas les sources techniques.
    $prompt_persona$,
    'Persona gestionnaire RH en SGCD (style seul : voix, acteurs, périmètre) — candidate A/B, non active par défaut (#300)',
    'generator',
    TRUE,
    'migration-issue-300',
    CURRENT_TIMESTAMP
